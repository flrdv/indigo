...
func TestRoute(t *testing.T) {
	...
	t.Run("GET /", func(t *testing.T) {
		...
		request.Path = "/"
		resp := http.AsBuilder(request, r.OnRequest(request))
...
func TestRouter_MethodNotAllowed(t *testing.T) {
	...
	request.Path = "/"
	request.Method = method.POST
	response := http.AsBuilder(request, r.OnRequest(request))
...
func TestRouter_RouteError(t *testing.T) {
	...
	t.Run("status.ErrBadRequest", func(t *testing.T) {
		request := getRequest()
		resp := http.AsBuilder(request, r.OnError(request, status.ErrBadRequest))
	...
	t.Run("status.ErrURIDecoding (also bad request)", func(t *testing.T) {
		request := getRequest()
		resp := http.AsBuilder(request, r.OnError(request, status.ErrURIDecoding))
		require.Equal(t, status.Teapot, resp.Code)
	...
	t.Run("unregistered http error", func(t *testing.T) {
		request := getRequest()
		resp := http.AsBuilder(request, r.OnError(request, status.ErrNotImplemented))
	...
	t.Run("unregistered ordinary error", func(t *testing.T) {
		request := getRequest()
		resp := http.AsBuilder(request, r.OnError(request, errors.New("any error text here")))
